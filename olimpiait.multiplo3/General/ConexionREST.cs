...
namespace olimpiait.multiplo3.General
{
    public class ConexionREST : IConexionRest
    {
        ...
        public async Task<HttpResponseWrapper<object>> Get(string url)
        {
            ...
            if (responseHTTP.IsSuccessStatusCode)
            {
                var response = await DeserializarRespuesta<object>(responseHTTP, OpcionesPorDefectoJSON);
        ...
        public async Task<HttpResponseWrapper<object>> PostFormData<TResponse>(string url, MultipartFormDataContent enviar)
        {
